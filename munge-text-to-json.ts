...
const munged = text
  ...
  .reduce((p, c) => {
    ...
    if(typeof currentScene.outline[currentScene.outline.length - 1] !== 'string') {
      ...
      //The last element in the outline was a stage direction
      const sd = currentScene.outline[currentScene.outline.length - 1] as StageDirection
      currentScene.outline[currentScene.outline.length - 1] = sd + line
    }
    return p
  }, [] as Scene[])
